...
object JoinUnit {
  ...
  def test(): Unit = {
    val query = sinq_pg.select(T_STUDENT.id, T_STUDENT.name, T_TEACHER.id, T_TEACHER.name).from(T_STUDENT).join(T_TEACHER).on(Eq(T_STUDENT.teacher, T_TEACHER.id))
      .where(condition).orderBy(ASC(T_STUDENT.id)).limit(10, 0)
