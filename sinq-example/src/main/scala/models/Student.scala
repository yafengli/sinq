...
case class Student(@BeanProperty var name: String, @BeanProperty var age: Int, @BeanProperty var address: String) {
  @Id @GeneratedValue(strategy = GenerationType.TABLE, generator = "seq_t_student") @TableGenerator(name = "seq_t_student", table = "seq_t_student", allocationSize = 1) @BeanProperty
  var id: Long = _

  @ManyToOne(fetch = FetchType.EAGER, optional = false) @JoinColumn(name = "teacher_id") @BeanProperty
