...
abstract class CQModel[T: Manifest] extends JPA {
  ...
  def get(id: Any): Option[T] = {
    withEntityManager(_.find(getType, id).asInstanceOf[T])
  }
  ...
  def fetch(limit: Int, offset: Int)(call: (CQExpression[T, T]) => Seq[Predicate]): Option[List[T]] = {
    withEntityManager(CQBuilder(_, getType.asInstanceOf[Class[T]], getType.asInstanceOf[Class[T]]).fetch(limit, offset)(call))

  }

  def single(call: (CQExpression[T, T]) => Seq[Predicate]): Option[T] = {
    withEntityManager(CQBuilder(_, getType.asInstanceOf[Class[T]], getType.asInstanceOf[Class[T]]).single(call))
  }

  def count(call: (CQExpression[T, Tuple]) => Seq[Predicate]): Option[Long] = {
    withEntityManager(CQBuilder(_, getType.asInstanceOf[Class[T]], classOf[java.lang.Long]).count(call))
  }

  def multi(selectCall: (CQExpression[T, Tuple]) => Seq[Selection[_]], call: (CQExpression[T, Tuple]) => Seq[Predicate]): Option[List[Tuple]] = {
    withEntityManager(CQBuilder(_, getType.asInstanceOf[Class[T]], classOf[Tuple]).multi(selectCall, call))
  }
}